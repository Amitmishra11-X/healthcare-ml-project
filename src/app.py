...
with st.sidebar:
    ...
    st.markdown("**B.Tech CSE | LDAH Engg. College**")
...
if page == "🏠 Home & Prediction":
    ...
    st.markdown("*AI-powered clinical decision support using Random Forest & XGBoost*")
    st.markdown("---")
    ...
    if not loaded:
        st.error("⚠️ Models not found. Please check that model files are in the models/ folder.")
        st.info(f"Looking in: {MODELS_DIR}")
        st.info(f"Files there: {os.listdir(MODELS_DIR) if os.path.exists(MODELS_DIR) else 'Folder not found'}")
    else:
        ...
        if st.button("🔮 Predict Diabetes Risk", use_container_width=True):
            ...
            c3.metric("Average", f"{avg_prob*100:.1f}%", "Risk")
            c4.metric("Result", "🔴 HIGH RISK" if prediction==1 else "🟢 LOW RISK")

            if prediction == 1:
                st.error(f"⚠️ HIGH RISK OF DIABETES — Probability: {avg_prob*100:.1f}% — Please consult a doctor.")
            else:
                st.success(f"✅ LOW RISK OF DIABETES — Probability: {avg_prob*100:.1f}% — Continue regular health monitoring.")

            # Feature contribution bar chart
            st.markdown("---")
            st.subheader("📊 Feature Values for This Patient")
            feat_df = pd.DataFrame({
                'Feature': cols,
                'Value': base_features
            })
            fig, ax = plt.subplots(figsize=(10, 4))
            colors = ['#2980B9'] * len(cols)
            ax.barh(feat_df['Feature'], feat_df['Value'], color=colors)
            ax.set_xlabel('Value')
            ax.set_title('Patient Feature Values', fontweight='bold')
            plt.tight_layout()
            st.pyplot(fig)
            plt.close()
...
# ══════════════════════════════════════════════════════
    ...
    for title, fname in images.items():
        ...
        if os.path.exists(path):
            ...
            st.info(f"{title}: Run notebook to generate this chart")
...
# ══════════════════════════════════════════════════════
    ...
    This project builds a complete ML pipeline for diabetes risk prediction.

    ### 🎯 Objective
    Predict diabetes risk from 8 clinical measurements with high accuracy
    and provide transparent AI explanations using SHAP.
